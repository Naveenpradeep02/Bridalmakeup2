const imageData = [
  {
    hair: {
      serviceName: "Hair Style",
      para: "From chic waves to traditional buns, we craft hairstyles that turn heads. Be it a wedding or a party, your hair will speak elegance.",
      image: [
        "./imgs/hair/hair (1).jpg",
        "./imgs/hair/hair (2).jpg",
        "./imgs/hair/hair (3).jpg",
        "./imgs/hair/hair (4).jpg",
        "./imgs/hair/hair (5).jpg",
        "./imgs/hair/hair (6).jpg",
        "./imgs/hair/hair (7).jpg",
        "./imgs/hair/hair (8).jpg",
        "./imgs/hair/hair (9).jpg",
        "./imgs/hair/hair (10).jpg",
      ],
    },
  },
  {
    bridal: {
      serviceName: "Bridal",
      para: "It's your day to glow! Our bridal makeup will make you look stunning in every click and moment. Feel confident walking down the aisle in style.",
      image: [
        "./imgs/bridal/bridal (1).jpg",
        "./imgs/bridal/bridal (2).jpg",
        "./imgs/bridal/bridal (3).jpg",
        "./imgs/bridal/bridal (4).jpg",
        "./imgs/bridal/bridal (5).jpg",
        "./imgs/bridal/bridal (6).jpg",
        "./imgs/bridal/bridal (7).jpg",
        "./imgs/bridal/bridal (8).jpg",
        "./imgs/bridal/bridal (9).jpg",
        "./imgs/bridal/bridal (10).jpg",
        "./imgs/bridal/bridal (11).jpg",
        "./imgs/bridal/bridal (12).jpg",
        "./imgs/bridal/bridal (13).jpg",
        "./imgs/bridal/bridal (14).jpg",
        "./imgs/bridal/bridal (15).jpg",
        "./imgs/bridal/bridal (16).jpg",
        "./imgs/bridal/bridal (17).jpg",
        "./imgs/bridal/bridal (18).jpg",
        "./imgs/bridal/bridal (19).jpg",
        "./imgs/bridal/bridal (20).jpg",
      ],
    },
  },
  {
    saree: {
      serviceName: "saree draping",
      para: "Elevate your style with perfectly draped sarees that bring out elegance and grace. Be it traditional or modern, we master every drape.",
      image: [
        "./imgs/sree/saree (6).jpg",
        "./imgs/sree/saree (7).jpg",
        "./imgs/sree/saree (8).jpg",
        "./imgs/sree/saree (12).jpg",
        "./imgs/sree/saree (13).jpg",
        "./imgs/sree/saree (14).jpg",
      ],
    },
    groom: {
      serviceName: "Groom makeup",
      image: [],
    },
    Puberty: {
      serviceName: "Puberty makeup",
      image: [],
    },
    Model: {
      serviceName: "Model photo shoot",
      para: "Look flawless under every light with our camera-ready makeup designed for photoshoots. Let your personality shine through every frame.",
      image: [
        "./imgs/model/model (2).jpg",
        "./imgs/model/model (3).jpg",
        "./imgs/model/model (4).jpg",
        "./imgs/model/model (5).jpg",
        "./imgs/model/model (1).jpeg",
      ],
    },
    Prewedding: {
      serviceName: "Prewedding photo shoot",
      para: "Capture the magic of your love story with our stunning pre-wedding photography. Let us create timeless memories that reflect your unique bond before your big day.",
      image: [
        "./imgs/preWed/prewed (1).jpg",
        "./imgs/preWed/prewed (8).jpg",
        "./imgs/preWed/prewed (9).jpg",
        "./imgs/preWed/prewed (10).jpg",
        "./imgs/preWed/prewed (11).jpg",
        "./imgs/preWed/prewed (12).jpg",
        "./imgs/preWed/prewed (13).jpg",
        "./imgs/preWed/prewed (14).jpg",
        "./imgs/preWed/prewed (15).jpg",
        "./imgs/preWed/prewed (16).jpg",
      ],
    },
    Baby: {
      serviceName: "Baby shower makeup",
      para: "Glow with joy and elegance on your special day with radiant, subtle makeup. Let your happiness shine through with our gentle touch.",
      image: [
        "./imgs/baby/baby (1).jpg",
        "./imgs/baby/baby (2).jpg",
        "./imgs/baby/baby (3).jpg",
        "./imgs/baby/baby (4).jpg",
        "./imgs/baby/baby (5).jpg",
        "./imgs/baby/baby (6).jpg",
        "./imgs/baby/baby (7).jpg",
        "./imgs/baby/baby (8).jpg",
        "./imgs/baby/baby (9).jpg",
        "./imgs/baby/baby (10).jpg",
        "./imgs/baby/baby (11).jpg",
      ],
    },
    portfolio: {
      serviceName: "",
      para: "",
      image: [
        "./imgs/hair/hair (1).jpg",
        "./imgs/hair/hair (2).jpg",
        "./imgs/hair/hair (3).jpg",
        "./imgs/hair/hair (4).jpg",
        "./imgs/hair/hair (5).jpg",
        "./imgs/hair/hair (6).jpg",
        "./imgs/hair/hair (7).jpg",
        "./imgs/hair/hair (8).jpg",
        "./imgs/hair/hair (9).jpg",
        "./imgs/hair/hair (10).jpg",
        "./imgs/bridal/bridal (1).jpg",
        "./imgs/bridal/bridal (2).jpg",
        "./imgs/bridal/bridal (3).jpg",
        "./imgs/bridal/bridal (4).jpg",
        "./imgs/bridal/bridal (5).jpg",
        "./imgs/bridal/bridal (6).jpg",
        "./imgs/bridal/bridal (7).jpg",
        "./imgs/bridal/bridal (8).jpg",
        "./imgs/bridal/bridal (9).jpg",
        "./imgs/bridal/bridal (10).jpg",
        "./imgs/bridal/bridal (11).jpg",
        "./imgs/bridal/bridal (12).jpg",
        "./imgs/bridal/bridal (13).jpg",
        "./imgs/bridal/bridal (14).jpg",
        "./imgs/bridal/bridal (15).jpg",
        "./imgs/bridal/bridal (16).jpg",
        "./imgs/bridal/bridal (17).jpg",
        "./imgs/bridal/bridal (18).jpg",
        "./imgs/bridal/bridal (19).jpg",
        "./imgs/bridal/bridal (20).jpg",
        "./imgs/sree/saree (1).jpg",
        "./imgs/sree/saree (6).jpg",
        "./imgs/sree/saree (7).jpg",
        "./imgs/sree/saree (8).jpg",
        "./imgs/sree/saree (9).jpg",
        "./imgs/sree/saree (10).jpg",
        "./imgs/sree/saree (11).jpg",
        "./imgs/sree/saree (12).jpg",
        "./imgs/sree/saree (13).jpg",
        "./imgs/sree/saree (14).jpg",
        "./imgs/model/model (2).jpg",
        "./imgs/model/model (3).jpg",
        "./imgs/model/model (4).jpg",
        "./imgs/model/model (5).jpg",
        "./imgs/model/model (1).jpeg",
        "./imgs/preWed/prewed (1).jpg",
        "./imgs/preWed/prewed (8).jpg",
        "./imgs/preWed/prewed (9).jpg",
        "./imgs/preWed/prewed (10).jpg",
        "./imgs/preWed/prewed (11).jpg",
        "./imgs/preWed/prewed (12).jpg",
        "./imgs/preWed/prewed (13).jpg",
        "./imgs/preWed/prewed (14).jpg",
        "./imgs/preWed/prewed (15).jpg",
        "./imgs/preWed/prewed (16).jpg",
        "./imgs/baby/baby (1).jpg",
        "./imgs/baby/baby (2).jpg",
        "./imgs/baby/baby (3).jpg",
        "./imgs/baby/baby (4).jpg",
        "./imgs/baby/baby (5).jpg",
        "./imgs/baby/baby (6).jpg",
        "./imgs/baby/baby (7).jpg",
        "./imgs/baby/baby (8).jpg",
        "./imgs/baby/baby (9).jpg",
        "./imgs/baby/baby (10).jpg",
        "./imgs/baby/baby (11).jpg",
        "./imgs/portfolio/img1 (1).png",
        "./imgs/portfolio/img1 (2).png",
        "./imgs/portfolio/img1 (3).png",
        "./imgs/portfolio/img1 (4).png",
        "./imgs/portfolio/27.jpg",
        "./imgs/portfolio/img1 (6).png",
        "./imgs/portfolio/img1 (7).png",
        "./imgs/portfolio/img1 (12).png",
        "./imgs/portfolio/img1 (8).png",
        "./imgs/portfolio/img1 (9).png",
        "./imgs/portfolio/img1 (10).png",
        "./imgs/portfolio/img1 (11).png",
        "./imgs/portfolio/img1 (13).png",
        "./imgs/portfolio/img1 (14).png",
        "./imgs/portfolio/img1 (15).png",
        "./imgs/portfolio/img1 (16).png",
        "./imgs/portfolio/img1 (17).png",
        "./imgs/portfolio/img1 (18).png",
        "./imgs/portfolio/img1 (19).png",
        "./imgs/portfolio/img1 (20).png",
        "./imgs/portfolio/img1 (21).png",
        "./imgs/portfolio/img1 (22).png",
        "./imgs/portfolio/img1 (24).png",
        "./imgs/portfolio/img1 (25).png",
        "./imgs/portfolio/1 (1).jpg",
        "./imgs/portfolio/1 (2).jpg",
        "./imgs/portfolio/1 (3).jpg",
        "./imgs/portfolio/1 (4).jpg",
        "./imgs/portfolio/1 (5).jpg",
      ],
    },
  },
];

const container = document.querySelector(".portfolio-container1");

const setImage = (category) => {
  const item = imageData.find((obj) => obj[category]);

  if (!item) {
    console.error("Category not found!");
    container.innerHTML = `<p class="error-message">Category not found!</p>`;
    return;
  }

  const imagedata = item[category];

  // Check if the image array is empty
  const imagesMarkup = imagedata.image.length
    ? imagedata.image.map((img) => `<img src="${img}" alt="Image" />`).join("")
    : `<p class="no-image">No Image Available</p>`;

  container.innerHTML = `
      <div class="portfolio-title">
        <img src="./imgs/rose.png" alt="Rose Icon" />
        <div  class="title-div" >
        <h3 class="profile-title">${imagedata.serviceName}</h3>
       
        </div>

        <img src="./imgs/rose.png" alt="Rose Icon" />
      </div>
       <p class="para-text">${imagedata.para}</P>
      <div class="imgage-container">
        ${imagesMarkup}
      </div>
    `;
};

// Retrieve category from localStorage and call setImage
const selectedCategory = localStorage.getItem("selectedCategory");
if (selectedCategory) {
  setImage(selectedCategory);
}
